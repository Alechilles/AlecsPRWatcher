...
test("refreshCodexReviewState preserves reactions before a polled new-head observation", async () => {
  const watches = await service();
  const github = new FakeCodexClient();
  await watches.registerWatch({ repo: "owner/repo", prNumber: 7 }, new Date("2026-04-27T12:00:00.000Z"));
  await watches.refreshCodexReviewState("owner/repo", github, 7, new Date("2026-04-27T12:01:00.000Z"));
...
test("refreshCodexReviewState does not complete when latest Codex review is not approved", async () => {
  const watches = await service();
  const github = new FakeCodexClient();
  github.reviews = [
    {
  ...
  assert.equal(watch.completionReason, undefined);
});
